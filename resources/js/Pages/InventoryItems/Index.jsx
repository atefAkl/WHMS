...
export default function Index({ items = [], categories = [] }) {
    ...
    const level3Categories = categories.filter(c => getCategoryLevel(c) === 3);
    ...
    const openCreateModal = () => {
        ...
        setData({
            name: "",
            category_id: level3Categories[0]?.id || "",
            image: null,
            is_active: true,
            variants: [{ name: "", quality: "", default_price: 0, is_active: true }],
            _method: "POST",
        });
    ...
    const confirmDelete = (e) => {
        ...
        router.post(route("inventory-items.destroy", itemToDelete.id), {
            _method: 'DELETE',
    ...
    return (
        <AuthenticatedLayout header={breadcrumbs}>
            <Head title={lang === "ar" ? "الأصناف المخزنية" : "Inventory Items"} />
                    ...
                    <div className="md:col-span-1 bg-surface border border-border p-4 shadow-sm rounded-none main-stack-y">
                        <div className="flex items-center justify-between border-b border-border pb-3 mb-3">
                            <span className="font-bold text-xs text-text flex items-center gap-1.5">
                                ...
                                {lang === "ar" ? "شجرة تصنيفات السلع" : "Categories Tree"}
                            </span>
                            {selectedCategoryId && (
                                <button
                        ...
                        <div className="space-y-1 text-xs">
                            <button
                                ...
                                className={`w-full text-start px-2 py-1.5 transition-colors flex items-center gap-2 rounded-none font-semibold ${
                                    !selectedCategoryId ? "bg-primary/10 text-primary border-l-2 border-primary" : "text-text hover:bg-surface-muted"
                                }`}
                            >
                                ...
                                {categoryTree.map((lvl1) => {
                                    ...
                                    return (
                                        <div key={lvl1.id} className="space-y-1">
                                            ...
                                            >
                                                {hasLvl2 ? (
                                                    isLvl1Expanded ? <FolderOpen className="h-3.5 w-3.5 text-amber-500 shrink-0" /> : <Folder className="h-3.5 w-3.5 text-amber-500 shrink-0" />
                                                ) : (
                                                    <Folder className="h-3.5 w-3.5 text-slate-400 shrink-0" />
                                                )}
                                                <span className="truncate">{lvl1.name}</span>
                                            </div>
                                            ...
                                            {hasLvl2 && isLvl1Expanded && (
                                                <div className="ps-4 space-y-1 border-s border-dashed border-border ms-3.5">
                                                    {lvl1.children.map((lvl2) => {
                                                        ...
                                                        const isLvl2Expanded = !!expandedCategories[lvl2.id];
                                                        ...
                                                        return (
                                                            <div key={lvl2.id} className="space-y-1">
                                                                ...
                                                                <div
                                                                    onClick={() => {
                                                                        if (hasLvl3) toggleExpand(lvl2.id);
                                                                        setSelectedCategoryId(lvl2.id);
                                                                    }}
                                                                    className={`w-full text-start px-2 py-1.5 transition-colors flex items-center gap-1.5 cursor-pointer rounded-none ${
                                                                ...
                                                                >
                                                                    {hasLvl3 ? (
                                                                        isLvl2Expanded ? <FolderOpen className="h-3.5 w-3.5 text-amber-400 shrink-0" /> : <Folder className="h-3.5 w-3.5 text-amber-400 shrink-0" />
                                                                    ) : (
                                                                        <Folder className="h-3.5 w-3.5 text-slate-400 shrink-0" />
                                                                    )}
                                                                    <span className="truncate">{lvl2.name}</span>
                                                                </div>

                                                                {/* Level 3 Children */}
                                                                {hasLvl3 && isLvl2Expanded && (
                                                                    <div className="ps-4 space-y-1 border-s border-dashed border-border ms-3.5">
                                                                        {lvl2.children.map((lvl3) => {
                                                                            const lvl3ItemCount = items.filter(i => i.category_id === lvl3.id).length;
                                                                            return (
                                                                                <div
                                                                                    key={lvl3.id}
                                                                                    onClick={() => setSelectedCategoryId(lvl3.id)}
                                                                                    className={`w-full text-start px-2 py-1 transition-colors flex items-center gap-1.5 cursor-pointer rounded-none text-[11px] ${
                                                                                        selectedCategoryId === lvl3.id 
                                                                                            ? "bg-primary/10 text-primary font-bold border-s-2 border-primary" 
                                                                                            : "text-text-muted hover:text-text hover:bg-surface-muted"
                                                                                    }`}
                                                                                >
                                                                                    <FileText className="h-3.5 w-3.5 text-slate-400 shrink-0" />
                                                                                    <span className="truncate">{lvl3.name}</span>
                    ...
                    <div className="md:col-span-3 main-stack-y">
                        ...
                        <div className="bg-surface border border-border p-3 flex items-center justify-between shadow-sm rounded-none">
                            <div className="text-xs">
                                <span className="text-text-muted">{lang === "ar" ? "التصنيف المحدد: " : "Active Category: "}</span>
                                <span className="font-bold text-text">
                                    {selectedCategoryId ? getCategoryPath(selectedCategoryId) : (lang === "ar" ? "كل الأصناف" : "All Categories")}
                                </span>
                                <span className="mx-2 text-text-muted">|</span>
                                <span className="text-text-muted">{lang === "ar" ? "العدد: " : "Count: "}</span>
                        ...
                        {filteredItems.length === 0 ? (
                            <div className="bg-surface border border-border p-12 text-center shadow-sm rounded-none">
                                ...
                                <h3 className="font-bold text-sm text-text">{lang === "ar" ? "لا توجد أصناف مخزنية" : "No Inventory Items"}</h3>
                                <p className="text-xs text-text-muted mt-1">
                                    {lang === "ar" ? "لم يتم العثور على أصناف في هذا التصنيف حالياً." : "No items have been added to this category yet."}
                                </p>
                            </div>
                        ) : viewMode === "grid" ? (
                            /* Primary Cards Grid */
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="space-y-1">
                                                ...
                                                <div className="text-[10px] text-text-muted truncate">
                                                    {lang === "ar" ? "الفئة: " : "Category: "}{getCategoryPath(item.category_id) || "—"}
                                                </div>
                                            </div>

                                            {/* Variants Badge Count */}
                                            <div className="flex items-center justify-between text-xs pt-2 border-t border-border">
                                                <span className="text-text-muted">{lang === "ar" ? "عدد الأشكال:" : "Variants count:"}</span>
                                                <span className="font-bold text-primary bg-primary/5 px-2 py-0.5 border border-primary/10 rounded-none">
                                                    {item.variants ? item.variants.length : 0} {lang === "ar" ? "أشكال" : "shapes"}
                                                </span>
                                            </div>
                                        </div>
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-sm text-start">
                                        <thead className="bg-surface-muted/50 text-text-muted text-[11px] font-bold uppercase tracking-wide border-b border-border">
                                            <tr>
                                                <th className="px-4 py-3 text-start w-16">{lang === "ar" ? "الصورة" : "Image"}</th>
                                                <th className="px-4 py-3 text-start">{lang === "ar" ? "الاسم" : "Name"}</th>
                                                <th className="px-4 py-3 text-start">{lang === "ar" ? "كود الصنف" : "Code"}</th>
                                                <th className="px-4 py-3 text-start">{lang === "ar" ? "الفئة" : "Category"}</th>
                                                <th className="px-4 py-3 text-center">{lang === "ar" ? "عدد الأشكال" : "Variants"}</th>
                                                <th className="px-4 py-3 text-center w-24">{lang === "ar" ? "الحالة" : "Status"}</th>
                                                <th className="px-4 py-3 text-end w-32">{lang === "ar" ? "الإجراءات" : "Actions"}</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-border">
                                            {filteredItems.map(item => (
                                                <tr key={item.id} className="hover:bg-surface-muted/30 transition-colors">
                                                    ...
                                                    <td className="px-4 py-3 font-bold text-text text-[13px]">{item.name}</td>
                                                    <td className="px-4 py-3 font-mono text-xs text-text-muted">{item.code}</td>
                                                    <td className="px-4 py-3 text-xs text-text-muted">{getCategoryPath(item.category_id) || "—"}</td>
                                                    <td className="px-4 py-3 text-center font-bold text-primary">{item.variants ? item.variants.length : 0}</td>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                >
                                    <option value="">
                                        {lang === "ar" ? "-- اختر فئة مستوى 3 --" : "-- Select Level 3 Category --"}
                                    </option>
                            ...
                            <div>
                                ...
                                {itemToEdit && itemToEdit.image && (
                                    <div className="mt-2 flex items-center gap-2">
                                        <span className="text-[10px] text-text-muted">{lang === "ar" ? "الصورة الحالية:" : "Current:"}</span>
                                        <img src={itemToEdit.image} alt={itemToEdit.name} className="h-10 w-10 object-cover border border-border rounded-none" />
                                    </div>
                                )}
                                <InputError message={errors.image || errors.image_file} className="mt-1" />
                            </div>
                        ...
                        <div className="border border-border p-4 bg-surface-muted/30 rounded-none main-stack-y">
                            <div className="flex items-center justify-between border-b border-border pb-2 mb-2">
                                <span className="font-bold text-xs text-text">
                                    {lang === "ar" ? "أشكال المنتج المتوفرة (Variants) *" : "Packaging Shapes/Variants *"}
                                </span>
                                <button
                                    type="button"
                            ...
                            <div className="space-y-3 max-h-[250px] overflow-y-auto pr-1">
                                {data.variants.map((variant, idx) => (
                                    <div key={idx} className="border border-border bg-surface p-3 space-y-2 relative rounded-none shadow-sm">
                                        ...
                                        <div className="grid grid-cols-2 gap-2 text-xs">
                                            <div>
                                                <label className="block text-[10px] font-bold text-text-muted mb-0.5">{lang === "ar" ? "الاسم (مثال: كرتون 3 كجم)" : "Name (e.g. Box 3kg)"} *</label>
                                                <input
                                                    type="text"
                                                    required
                                            ...
                                            <div>
                                                <label className="block text-[10px] font-bold text-text-muted mb-0.5">{lang === "ar" ? "الجودة (مثال: ممتاز)" : "Quality (e.g. Premium)"}</label>
                                                <input
                                                    type="text"
                                                    className="w-full text-xs border-border bg-surface text-text focus:ring-primary focus:border-primary rounded-none p-1.5"
                                                    value={variant.quality}
                                                    onChange={(e) => handleVariantChange(idx, "quality", e.target.value)}
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-2 text-xs">
                                            <div>
                                                <label className="block text-[10px] font-bold text-text-muted mb-0.5">{lang === "ar" ? "السعر الافتراضي" : "Default Price"} *</label>
                                                <input
                                                    type="number"
                                                    min="0"
                                                    step="0.01"
                                                    ...
                                                    onChange={(e) => handleVariantChange(idx, "default_price", e.target.value)}
                                                />
                                            </div>
                                            <div className="flex items-end pb-2">
                                                <label className="flex items-center gap-1.5 cursor-pointer select-none">
                                                    ...
                                                    />
                                                    <span className="text-[10px] font-medium text-text">{lang === "ar" ? "نشط" : "Active"}</span>
                                                </label>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <InputError message={errors.variants} className="mt-1 text-xs" />
                        </div>
                    </div>
            ...
            <Modal
                ...
                maxWidth="sm"
            >
                <form onSubmit={confirmDelete} className="p-6 text-center space-y-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-danger/80 font-bold bg-danger/5 p-2 border border-danger/10 mt-2 rounded-none">
                            {lang === "ar"
                                ? `الصنف المستهدف: ${itemToDelete?.name}`
                                : `Target Item: ${itemToDelete?.name}`}
                        </p>
                    </div>

                    {/* Secure Deletion Password Input */}
                    <div className="text-start space-y-1">
                        <InputLabel value={lang === "ar" ? "كلمة مرور الحفظ/الحذف الآمنة للمدير *" : "Secure Deletion Password *"} />
                        <TextInput
                            type="password"
                            required
                            placeholder={lang === "ar" ? "أدخل كلمة مرور الحذف الآمنة" : "Enter Secure password"}
                    ...
                    <div className="flex justify-center gap-3 pt-2">
                        ...
                        <DangerButton type="submit" className="rounded-none">
                            {lang === "ar" ? "تأكيد الحذف الآمن" : "Confirm Secure Delete"}
                        </DangerButton>
                    </div>
                </form>
            </Modal>
