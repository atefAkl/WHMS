...
export default function Show({ reception }) {
    ...
    const breadcrumbs = (
        <div className="flex items-center gap-[6px] text-xs text-text-muted">
            ...
            <ChevronRight className={`h-3.5 w-3.5 ${lang === "ar" ? "rotate-180" : ""}`} />
            <span className="text-primary font-medium">{reception.serial_number}</span>
        </div>
    );

    return (
        <AuthenticatedLayout header={breadcrumbs}>
            <Head title={lang === "ar" ? `تفاصيل سند الاستلام: ${reception.serial_number}` : `Reception Voucher Details: ${reception.serial_number}`} />
                ...
                <PageHeader
                    ...
                    title={
                        <div className="flex items-center gap-3">
                            <span className="font-extrabold text-lg text-text">{lang === "ar" ? `إيصال استلام: ${reception.serial_number}` : `Reception Receipt: ${reception.serial_number}`}</span>
                            <span
                    ...
                    actions={
                        <div className="flex flex-wrap items-center gap-1.5">
                            {reception.status === "draft" && (
                                <>
                                    <Tooltip text={lang === "ar" ? "تعديل السند" : "Edit Voucher"}>
                                        <Link
                                    ...
                                    <Tooltip text={lang === "ar" ? "اعتماد السند" : "Approve Voucher"}>
                                        <button
                                            onClick={() => {
                                                setErrorMsg("");
                            ...
                            {reception.status === "approved" && (
                                <Tooltip text={lang === "ar" ? "إعادة فتح السند" : "Reopen Voucher"}>
                                    <button
                                        onClick={() => {
                                            setErrorMsg("");
                                            setSecurePassword("");
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-surface border border-border p-5 shadow-sm rounded-none">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 text-xs">
                                <div>
                                    <span className="text-text-muted block font-medium mb-0.5">{lang === "ar" ? "العميل:" : "Customer:"}</span>
                                    <span className="text-text font-bold text-sm">{reception.customer?.name}</span>
                                </div>
                                ...
                                <div>
                                    ...
                                    {reception.contract ? (
                                        <Link 
                                            href={route("contracts.show", reception.contract_id)}
                                ...
                                <div>
                                    <span className="text-text-muted block font-medium mb-0.5">{lang === "ar" ? "الفترة الإلزامية:" : "Billing Period:"}</span>
                                    <span className="text-text font-semibold">
                                        {lang === "ar" ? "الفترة" : "Period"} {reception.period?.period_number}{" "}
                                        <span className="text-text-muted font-mono font-normal">
                                            ({reception.period?.start_date} {lang === "ar" ? "إلى" : "to"} {reception.period?.end_date})
                                        </span>
                                    </span>
                                </div>

                                <div>
                                    <span className="text-text-muted block font-medium mb-0.5">{lang === "ar" ? "تاريخ الاستلام الفعلي:" : "Actual Date:"}</span>
                                ...
                                <div>
                                    <span className="text-text-muted block font-medium mb-0.5">{lang === "ar" ? "مندوب العميل (المستلم منه):" : "Customer Representative:"}</span>
                                    {reception.representative ? (
                                        <span className="text-text font-semibold">
                                            {reception.representative.name}{" "}
                                            <span className="text-text-muted font-mono font-normal">
                                                ({reception.representative.phone_number})
                                            </span>
                                        </span>
                                    ) : (
                                ...
                                <div>
                                    <span className="text-text-muted block font-medium mb-0.5">{lang === "ar" ? "آخر تعديل بواسطة:" : "Last Updated By:"}</span>
                                    <span className="text-text font-semibold">
                                        {reception.editor?.name || "—"}{" "}
                                        <span className="text-text-muted font-mono font-normal text-[10px]">
                                            ({new Date(reception.updated_at).toLocaleString(lang === "ar" ? "ar-EG" : "en-US")})
                                        </span>
                                    </span>
                                </div>
                            </div>
                        ...
                        <div className="bg-surface border border-border p-5 shadow-sm rounded-none">
                            <div className="flex items-center gap-2 border-b border-border pb-3 mb-4">
                                ...
                                <h3 className="font-bold text-xs text-primary uppercase tracking-wider">
                                    {lang === "ar" ? "البضائع والكميات المستلمة" : "Items & Pallets List"}
                                </h3>
                            </div>
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-xs text-start">
                                    ...
                                    <tbody className="divide-y divide-border">
                                        {reception.inventory_entries?.map((entry, index) => (
                    ...
                    <div className="lg:col-span-1 space-y-6">
                            ...
                            {reception.driver ? (
                                <div className="space-y-3 text-xs">
                                    <div>
                                        ...
                                        <span className="text-text font-bold">{reception.driver.name}</span>
                                    </div>
                                    <div>
                                        <span className="text-text-muted block font-medium">{lang === "ar" ? "رقم الجوال:" : "Phone Number:"}</span>
                                        <span className="text-text font-mono font-bold">{reception.driver.phone_number || "—"}</span>
                                    </div>
                                    <div>
                                        <span className="text-text-muted block font-medium">{lang === "ar" ? "رقم الهوية / الإقامة:" : "ID / Iqama No.:"}</span>
                                        <span className="text-text font-mono font-bold">{reception.driver.id_number || "—"}</span>
                                    </div>
                                    <div>
                                        <span className="text-text-muted block font-medium">{lang === "ar" ? "رقم اللوحة:" : "Plate Number:"}</span>
                                        <span className="text-text font-bold font-mono">{reception.driver.vehicle_plate || "—"}</span>
                                    </div>
                                    <div>
                                        <span className="text-text-muted block font-medium">{lang === "ar" ? "نوع السيارة:" : "Vehicle Type:"}</span>
                                        <span className="text-text font-semibold">{reception.driver.vehicle_type || "—"}</span>
                                    </div>
                                    <div>
                                        <span className="text-text-muted block font-medium">{lang === "ar" ? "رقم رخصة السير:" : "License No.:"}</span>
                                        <span className="text-text font-mono">{reception.driver.license_number || "—"}</span>
                                    </div>
                                </div>
                            ) : (
                        ...
                        <div className="bg-surface border border-border p-5 shadow-sm rounded-none">
                            ...
                            {reception.history && reception.history.length > 0 ? (
                ...
                <form onSubmit={handleDelete} className="p-6 space-y-4 text-start" dir={lang === "ar" ? "rtl" : "ltr"}>
                    <div className="flex items-center gap-2 border-b border-border pb-3">
                        ...
                        <h3 className="font-bold text-lg text-text">
                            {lang === "ar" ? "تأكيد حذف سند الاستلام" : "Confirm Voucher Deletion"}
                        </h3>
                    </div>
