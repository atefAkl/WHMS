...
export default function CreateEdit({ 
    ...
    useEffect(() => {
        if (data.contract_id) {
            ...
            if (contract) {
                ...
                axios.get(route('api.contracts.occupancy-stats', contract.id))
                    .then(res => {
    ...
    const filteredPOSItems = inventoryItems.filter(item => 
    ...
    const selectedPOSItem = inventoryItems.find(i => i.id === parseInt(posItemId));
    ...
    useEffect(() => {
        if (posVariantId && posVariants.length > 0) {
            const variant = posVariants.find(v => v.id === parseInt(posVariantId));
            if (variant) {
    ...
    const handlePalletBlur = () => {
        ...
        axios.get(route('api.pallets.lookup', { pallet_number: number }))
            .then(res => {
            ...
            .catch(err => {
                setLoadingPallet(false);
                console.error(err);
                setPosRowError(lang === "ar" ? "خطأ في الاتصال بالخادم للتحقق من الطبلية." : "Error checking pallet code.");
            });
    };
        ...
        axios.post(route("api.drivers.store"), newDriverData)
            ...
            .catch(err => {
                ...
                if (err.response?.data?.message) {
                    ...
                    setDriverError(lang === "ar" ? "تعذر إضافة السائق. يرجى التحقق من المدخلات." : "Could not add driver.");
                }
            });
    };
    ...
    const handleAddPOSRow = () => {
        ...
        if (!posPalletNumber.trim()) {
            setPosRowError(lang === "ar" ? "يجب إدخال رقم الطبلية." : "Pallet number is required.");
            palletInputRef.current?.focus();
            return;
        }
        if (!posItemId) {
            setPosRowError(lang === "ar" ? "يجب اختيار الصنف المخزني." : "Inventory item is required.");
            itemSearchRef.current?.focus();
            return;
        }
        if (!posVariantId) {
            setPosRowError(lang === "ar" ? "يجب اختيار عبوة الصنف." : "Variant is required.");
            variantSelectRef.current?.focus();
            return;
        }
        if (!posQuantity || parseFloat(posQuantity) <= 0) {
            setPosRowError(lang === "ar" ? "الكمية يجب أن تكون أكبر من الصفر." : "Quantity must be greater than zero.");
    ...
    const breadcrumbs = (
        <div className="flex items-center gap-[6px] text-xs text-text-muted">
            ...
            <Link href={route("receptions.index")} className="hover:text-primary transition-colors">
                {lang === "ar" ? "سندات الاستلام" : "Reception Vouchers"}
            </Link>
            <ChevronRight className={`h-3.5 w-3.5 ${lang === "ar" ? "rotate-180" : ""}`} />
            <span className="text-primary font-medium">
                {isEdit ? (lang === "ar" ? "تعديل سند" : "Edit Voucher") : (lang === "ar" ? "إنشاء سند جديد" : "New Voucher")}
            </span>
        </div>
    );
    ...
    return (
        <AuthenticatedLayout header={breadcrumbs}>
            <Head title={isEdit ? (lang === "ar" ? "تعديل سند استلام" : "Edit Reception Voucher") : (lang === "ar" ? "إنشاء سند استلام" : "New Reception Voucher")} />

            <div className="max-w-6xl mx-auto pb-12 main-stack-y" dir={lang === "ar" ? "rtl" : "ltr"}>
                
                {/* Page header title bar */}
                <PageHeader
                    icon={Layers}
                    title={isEdit ? (lang === "ar" ? `تعديل إيصال استلام: ${reception.serial_number}` : `Edit Receipt: ${reception.serial_number}`) : (lang === "ar" ? "إنشاء إيصال استلام جديد" : "Create New Reception Receipt")}
                    description={
                    ...
                    actions={
                        <div className="flex items-center gap-2">
                            ...
                            <Tooltip text={lang === "ar" ? "رجوع" : "Back"}>
                                <Link
                                    href={isEdit ? route("receptions.show", reception.id) : route("receptions.index")}
                    ...
                    <div className="bg-amber-500/5 border border-amber-500/25 p-3 rounded-none flex flex-wrap items-center gap-3">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                            {draftReceptions.map((draft) => {
                                ...
                                return (
                                    <Tooltip key={draft.id} text={draftTooltipText}>
                                        <Link
                                            href={route("receptions.edit", draft.id)}
                    ...
                    <div className="lg:col-span-3 space-y-6">
                        ...
                        <div className="bg-surface border border-border p-5 shadow-sm rounded-none space-y-4">
                            ...
                            <div className="flex items-center justify-between border-b border-border pb-3">
                                <h3 className="font-bold text-xs text-primary uppercase tracking-wider">
                                    {lang === "ar" ? "معلومات السند الأساسية" : "Voucher General Information"}
                                </h3>
                                <button
                                    type="button"
                                    ...
                                    title={lang === "ar" ? "طي/عرض معلومات السند" : "Collapse/Expand Voucher Info"}
                                >
                                    {isGeneralCollapsed ? (
                            ...
                            {!isGeneralCollapsed && (
                                ...
                                <div>
                                    ...
                                    <TextInput
                                        ...
                                        value={reception?.serial_number || (lang === "ar" ? "سيتم توليده تلقائياً" : "Auto-generated")}
                                        disabled
                                        readOnly
                                    />
                                </div>
                                ...
                                {/* Contract Select */}
                                <div>
                                    <InputLabel value={lang === "ar" ? "العقد المرتبط *" : "Linked Contract *"} />
                                    <select
                                        ...
                                        disabled={(reception?.status === 'approved') || !data.customer_id}
                                        required
                                    >
                                        <option value="">{lang === "ar" ? "-- اختر العقد --" : "-- Select Contract --"}</option>
                                        {availableContracts.map((c) => (
                                            <option key={c.id} value={c.id}>
                                                {c.contract_number} (حتى {c.end_date})
                                            </option>
                                        ))}
                                    </select>
                                ...
                                {/* Period Select */}
                                <div>
                                    <InputLabel value={lang === "ar" ? "فترة الفوترة المرتبطة *" : "Linked Period *"} />
                                    <select
                                        className="mt-1 block w-full border-border bg-surface text-text text-sm focus:border-primary focus:ring-primary rounded-none h-[42px] px-3"
                                    ...
                                    >
                                        ...
                                        {availablePeriods.map((p) => (
                                            <option key={p.id} value={p.id}>
                                                {lang === "ar" ? "فترة" : "Period"} {p.period_number} ({p.start_date} إلى {p.end_date})
                                            </option>
                                        ))}
                                    </select>
                                ...
                                <div>
                                    ...
                                    >
                                        <option value="">{lang === "ar" ? "-- لا يوجد سائق مسجل --" : "-- No Driver --"}</option>
                                        {driverList.map((d) => (
                                ...
                                <div>
                                    <InputLabel value={lang === "ar" ? "مندوب العميل (المستلم منه)" : "Customer Agent"} />
                                    <select
                                        className="mt-1 block w-full border-border bg-surface text-text text-sm focus:border-primary focus:ring-primary rounded-none h-[42px] px-3"
                                        ...
                                        disabled={!data.contract_id}
                                    >
                                        <option value="">{lang === "ar" ? "-- لا يوجد مندوب محدد --" : "-- No Representative --"}</option>
                                ...
                                <div>
                                    <InputLabel value={lang === "ar" ? "المزرعة أو المصدر" : "Farm / Source"} />
                                    <TextInput
                                        type="text"
                                        className="mt-1 w-full text-sm rounded-none border-border"
                                        placeholder={lang === "ar" ? "مصدر البضاعة (مثال: مزرعة القصيم)" : "e.g. Al-Qassim Farm"}
                                        value={data.farm_source}
                                        onChange={(e) => setData("farm_source", e.target.value)}
                                    />
                                    <InputError message={errors.farm_source} className="mt-1" />
                                </div>

                                {/* Date Input */}
                                <div>
                                    <InputLabel value={lang === "ar" ? "تاريخ الاستلام الفعلي *" : "Reception Date *"} />
                                    <TextInput
                                        type="date"
                                        className="mt-1 w-full text-sm rounded-none border-border"
                                        value={data.reception_date}
                                        onChange={(e) => setData("reception_date", e.target.value)}
                                        required
                                    />
                                    <InputError message={errors.reception_date} className="mt-1" />
                                </div>
                            </div>

                            {/* Notes (الملاحظات) */}
                            <div className="pt-2">
                                <InputLabel value={lang === "ar" ? "ملاحظات السند" : "Voucher Notes"} />
                                <textarea
                                    ...
                                    placeholder={lang === "ar" ? "اكتب أي ملاحظات أو تفاصيل إضافية حول هذا السند..." : "Write any additional notes or details about this voucher..."}
                                />
                            ...
                            {isEdit && (
                                <div className="pt-2">
                                    <InputLabel value={lang === "ar" ? "سبب التعديل (يسجل في سجل المراجعة)" : "Modification Reason"} />
                                    <TextInput
                                        className="mt-1 w-full text-sm rounded-none border-border"
                                        value={data.modification_reason}
                                        onChange={(e) => setData("modification_reason", e.target.value)}
                                        placeholder={lang === "ar" ? "اكتب سبب تعديل هذا السند بالتفصيل..." : "Write why you are editing..."}
                                        required={false}
                                    />
                                    <InputError message={errors.modification_reason} className="mt-1" />
                                </div>
                            )}
                                </>
                            )}
                        ...
                        <div className="bg-surface border border-primary/20 p-5 shadow-sm rounded-none space-y-4">
                            <h3 className="font-bold text-xs text-primary border-b border-border pb-2 uppercase tracking-wider flex items-center gap-1.5">
                                ...
                                {lang === "ar" ? "إدخال البند السريع (POS Bar)" : "Quick POS Item Entry"}
                            </h3>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
                                ...
                                <div className="sm:col-span-2">
                                    <InputLabel value={lang === "ar" ? "رقم الطبلية *" : "Pallet No. *"} />
                                    <input
                                        ref={palletInputRef}
                                        type="text"
                                        ...
                                        onKeyDown={(e) => handleKeyNavigation(e, sizeSelectRef, null)}
                                ...
                                {/* Size Select */}
                                <div className="sm:col-span-2">
                                    <InputLabel value={lang === "ar" ? "الحجم والنوع *" : "Size & Type *"} />
                                    <select
                                        ref={sizeSelectRef}
                                        ...
                                        value={posPalletSize}
                                        onChange={(e) => setPosPalletSize(e.target.value)}
                                        onKeyDown={(e) => handleKeyNavigation(e, itemSearchRef, palletInputRef)}
                                        required
                                    >
                                ...
                                <div className="sm:col-span-3 relative">
                                    <InputLabel value={lang === "ar" ? "الصنف المخزني *" : "Inventory Item *"} />
                                    <input
                                        ref={itemSearchRef}
                                        type="text"
                                        className="mt-1 block w-full text-xs rounded-none border-border h-[38px] px-2 bg-surface text-text"
                                        ...
                                        onFocus={() => setShowPosItemDropdown(true)}
                                        onBlur={() => setShowPosItemDropdown(false)}
                                        onKeyDown={(e) => {
                                            if (showPosItemDropdown && filteredPOSItems.length > 0) {
                                                if (e.key === "ArrowDown") {
                                                    e.preventDefault();
                                                    setPosItemActiveIndex(prev => 
                                                        prev < filteredPOSItems.length - 1 ? prev + 1 : 0
                                                    );
                                                    return;
                                                } else if (e.key === "ArrowUp") {
                                                    e.preventDefault();
                                                    setPosItemActiveIndex(prev => 
                                                        prev > 0 ? prev - 1 : filteredPOSItems.length - 1
                                                    );
                                                    return;
                                                } else if (e.key === "Enter") {
                                                    if (posItemActiveIndex >= 0 && posItemActiveIndex < filteredPOSItems.length) {
                                                        e.preventDefault();
                                                        const selected = filteredPOSItems[posItemActiveIndex];
                                                        setPosItemId(selected.id);
                                                        setPosItemSearch(displayBilingual(selected.name));
                                            ...
                                            handleKeyNavigation(e, variantSelectRef, sizeSelectRef);
                                        }}
                                        required

                                    />
                                    {showPosItemDropdown && posItemSearch && filteredPOSItems.length > 0 && (
                                        <div className="absolute z-20 w-full bg-surface border border-border shadow-md max-h-48 overflow-y-auto mt-1 divide-y divide-border">
                                            {filteredPOSItems.map((item, idx) => (
                                                <div
                                                    ...
                                                    onMouseDown={(e) => {
                                                        e.preventDefault();
                                                        setPosItemId(item.id);
                                                        setPosItemSearch(displayBilingual(item.name));
                                                        setShowPosItemDropdown(false);
                                                        variantSelectRef.current?.focus();
                                                    }}
                                                >
                                                    {displayBilingual(item.name)} ({item.code})
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                ...
                                {/* Variant Select (عبوة) */}
                                <div className="sm:col-span-2">
                                    <InputLabel value={lang === "ar" ? "العبوة *" : "Variant *"} />
                                    <select
                                        ref={variantSelectRef}
                                        className="mt-1 block w-full border-border bg-surface text-text text-xs focus:border-primary focus:ring-primary rounded-none h-[38px] px-2"
                                        value={posVariantId}
                                        onChange={(e) => setPosVariantId(e.target.value)}
                                        onKeyDown={(e) => handleKeyNavigation(e, qtyInputRef, itemSearchRef)}
                                        disabled={!posItemId}
                                        required
                                    >
                                        <option value="">{lang === "ar" ? "-- عبوة --" : "-- Variant --"}</option>
                                        {posVariants.map((v) => (
                                            <option key={v.id} value={v.id}>
                                                {displayBilingual(v.name)}{v.quality ? ` - ${displayBilingual(v.quality)}` : ""}
                                            </option>
                                        ))}
                                ...
                                <div className="sm:col-span-1">
                                    <InputLabel value={lang === "ar" ? "الدرجة" : "Quality"} />
                                    <TextInput
                                        ref={qualityInputRef}
                                    ...
                                    <input
                                        ...
                                        onKeyDown={(e) => {
                                            if (e.key === "Enter") {
                                                ...
                                                handleKeyNavigation(e, addButtonRef, variantSelectRef);
                                            }
                                        }}
                                        required
                                    />
                                ...
                                <div className="sm:col-span-1">
                                    <button
                                        ...
                                        onKeyDown={(e) => handleKeyNavigation(e, null, qtyInputRef)}
                                    >
                        ...
                        <div className="bg-surface border border-border p-5 shadow-sm rounded-none space-y-4">
                            <h3 className="font-bold text-xs text-primary border-b border-border pb-2 uppercase tracking-wider">
                                {lang === "ar" ? "البنود المدرجة حالياً بالسند" : "Pallets List in this Voucher"}
                            </h3>
                                ...
                                <table className="w-full text-xs text-start border border-border">
                                    <thead className="bg-surface-muted text-text-muted font-bold border-b border-border">
                                        <tr>
                                            ...
                                            <th className="px-3 py-2 text-start">{lang === "ar" ? "حجم/نوع الطبلية" : "Pallet Size"}</th>
                                            ...
                                            <th className="px-3 py-2 text-end">{lang === "ar" ? "الكمية المستلمة" : "Qty Received"}</th>
                                        ...
                                        {data.items.length === 0 ? (
                                            <tr>
                                                <td colSpan="8" className="px-3 py-6 text-center text-text-muted font-bold">
                                                    {lang === "ar" ? "لا توجد بنود مضافة لهذا السند بعد. استخدم الشريط السريع بالأعلى لإدخال البنود." : "No items added yet. Use the POS bar to add items."}
                                                </td>
                                            </tr>
                                        ) : (
                                            data.items.map((item, index) => {
                                                ...
                                                return (
                                                    <tr key={index} className="hover:bg-slate-50 transition-colors">
                                                        <td className="px-3 py-2.5 font-mono text-text-muted">{index + 1}</td>
                                                        <td className="px-3 py-2.5 font-mono font-bold text-primary">{item.pallet_number}</td>
                                                        <td className="px-3 py-2.5 font-bold text-text-muted">{item.pallet_size}</td>
                            ...
                            {errors.items && <p className="text-xs text-danger font-bold mt-1">{errors.items}</p>}
                        </div>
                    </div>
                        ...
                        <div className="bg-surface border border-border p-5 shadow-sm rounded-none space-y-4">
                            <div className="flex items-center justify-between border-b border-border pb-3">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h3 className="font-bold text-xs text-primary uppercase tracking-wider">
                                        {lang === "ar" ? "سعة وحالة إشغال العقد" : "Contract Capacity Stats"}
                                    </h3>
                                </div>
                                <button
                                    type="button"
                                ...
                                <>
                                    {loadingStats ? (
                                        <div className="py-6 text-center text-xs text-text-muted">
                                            {lang === "ar" ? "جاري تحميل إحصاءات السعة..." : "Loading stats..."}
                                        </div>
                                    ) : contractStats ? (
                                        <div className="space-y-4 text-xs">
                                            <div className="bg-slate-50 border border-border p-3 space-y-3">
                                                <div className="flex justify-between items-center">
                                                    <span className="text-text-muted font-medium">{lang === "ar" ? "السعة الكلية للعقد:" : "Total Capacity:"}</span>
                                                    <span className="font-bold text-text font-mono text-sm">{contractStats.total_capacity} {lang === "ar" ? "طبلية" : "Pallets"}</span>
                                                </div>
                                                <div className="flex justify-between items-center">
                                                    <span className="text-text-muted font-medium">{lang === "ar" ? "المستلم بالمخازن:" : "Received in Store:"}</span>
                                                    <span className="font-bold text-amber-600 font-mono text-sm">{liveCurrentlyInWarehouse} {lang === "ar" ? "طبلية" : "Pallets"}</span>
                                                </div>
                                                <hr className="border-border" />
                                                <div className="flex justify-between items-center">
                                                    <span className="text-text-muted font-bold">{lang === "ar" ? "السعة المتبقية:" : "Remaining Space:"}</span>
                                                    <span className={`font-black font-mono text-[15px] ${liveRemaining > 0 ? "text-emerald-600" : "text-danger"}`}>
                                                        {liveRemaining} {lang === "ar" ? "طبلية" : "Pallets"}
                                                    </span>
                                                </div>
                                            </div>
                                        ...
                                        <div className="py-4 text-center text-xs text-text-muted">
                                            {lang === "ar" ? "يرجى تحديد العقد لإظهار إحصاءات الإشغال." : "Select contract to calculate capacity stats."}
                                        </div>
                                    )}
                                </>
                            )}
            ...
            <Modal show={isDriverModalOpen} onClose={() => setDriverModalOpen(false)} maxWidth="md">
                <form onSubmit={handleCreateDriver} className="p-6 space-y-4 text-start" dir={lang === "ar" ? "rtl" : "ltr"}>
                    <div className="flex items-center justify-between border-b border-border pb-3">
                        <h3 className="font-bold text-lg text-text">
                            {lang === "ar" ? "إضافة سائق ناقل جديد" : "Add New Carrier Driver"}
                        </h3>
                        <button type="button" onClick={() => setDriverModalOpen(false)} className="text-text-muted hover:text-text">
                            <X className="h-5 w-5" />
                        </button>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            <InputLabel value={lang === "ar" ? "اسم السائق *" : "Driver Name *"} />
                            <TextInput
                        ...
                        <div>
                            <InputLabel value={lang === "ar" ? "رقم الجوال" : "Phone Number"} />
                            <TextInput
                                className="mt-1 w-full text-xs rounded-none border-border"
                                value={newDriverData.phone_number}
                                onChange={(e) => setNewDriverData(prev => ({ ...prev, phone_number: e.target.value }))}
                            />
                        </div>

                        <div>
                            <InputLabel value={lang === "ar" ? "رقم الهوية / الإقامة" : "ID / Iqama Number"} />
                            <TextInput
                                className="mt-1 w-full text-xs rounded-none border-border"
                                value={newDriverData.id_number}
                                onChange={(e) => setNewDriverData(prev => ({ ...prev, id_number: e.target.value }))}
                            />
                        </div>

                        <div>
                            <InputLabel value={lang === "ar" ? "رقم اللوحة" : "Vehicle Plate No."} />
                            <TextInput
                                className="mt-1 w-full text-xs rounded-none border-border"
                                value={newDriverData.vehicle_plate}
                                onChange={(e) => setNewDriverData(prev => ({ ...prev, vehicle_plate: e.target.value }))}
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <TextInput
                                ...
                                onChange={(e) => setNewDriverData(prev => ({ ...prev, vehicle_type: e.target.value }))}
                                placeholder="دينا، تريلا، لوري..."
                            />
                        </div>

                        <div>
                            <InputLabel value={lang === "ar" ? "رقم رخصة القيادة" : "Driver License No."} />
                            <TextInput
                                className="mt-1 w-full text-xs rounded-none border-border"
                    ...
                    {driverError && <p className="text-xs text-danger font-bold">{driverError}</p>}
                    ...
                    <div className="flex items-center justify-end gap-2 border-t border-border pt-4">
                        ...
                        <Tooltip text={lang === "ar" ? "إضافة السائق" : "Add Driver"}>
                            <button
                                type="submit"
                                ...
                                {showButtonText && (
                                    <span>
                                        {addingDriver
                                            ...
                                            : (lang === "ar" ? "إضافة السائق" : "Add Driver")}
                                    </span>
                                )}
                            </button>
                        </Tooltip>
                    ...
                    {reception && (
                        <div className="bg-surface-muted/50 p-3 border border-border text-xs font-mono rounded-none">
                            <div>
                                ...
                                <span className="text-text font-bold">{reception.serial_number}</span>
                            </div>
                            {reception.customer && (
                                <div className="mt-1">
                                    <span className="font-bold text-text-muted">{lang === "ar" ? "العميل: " : "Customer: "}</span>
                                    <span className="text-text font-bold">{reception.customer.name}</span>
                                </div>
                            )}
                        </div>
                    )}
