...
export default function CreateEdit({ 
    ...
    useEffect(() => {
        const generalFields = ['customer_id', 'contract_id', 'period_id', 'written_reference', 'delivery_date', 'driver_id', 'representative_id', 'exit_authorization_id'];
    ...
    useEffect(() => {
        if (data.customer_id) {
            ...
            if (customer) {
                setAvailableContracts(customer.contracts || []);
                if (!isEdit && !customer.contracts?.some(c => c.id === parseInt(data.contract_id))) {
                    setData(d => ({
                        ...d,
                        contract_id: "",
                        period_id: "",
    ...
    const loadPallets = (contractId) => {
        ...
        axios.get(route('api.contracts.pallets', contractId))
            .then(res => {
    ...
    useEffect(() => {
        if (data.contract_id) {
            const contract = availableContracts.find(c => c.id === parseInt(data.contract_id));
            if (contract) {
                ...
                if (!isEdit && !data.period_id) {
                    setData(d => ({
                        ...d,
                        period_id: contract.periods?.[0]?.id || "",
    ...
    useEffect(() => {
        if (posPalletId && data.contract_id) {
            const selectedPallet = posPallets.find(p => p.id === parseInt(posPalletId));
            if (selectedPallet) {
            ...
            axios.get(route('api.contracts.pallets.items', [data.contract_id, posPalletId]))
                .then(res => {
    ...
    useEffect(() => {
        if (posItemId && posPalletId && data.contract_id) {
            ...
            axios.get(route('api.contracts.pallets.items.variants', [data.contract_id, posPalletId, posItemId]))
                .then(res => {
    ...
    useEffect(() => {
        if (posVariantId && posVariants.length > 0) {
            const selectedVar = posVariants.find(v => v.inventory_item_variant_id === parseInt(posVariantId));
            if (selectedVar) {
                setPosMaxBalance(selectedVar.balance);
                setPosQuality(displayBilingual(selectedVar.variant?.quality) || "");
    ...
    useEffect(() => {
        if (data.exit_authorization_id) {
            const exitAuth = exitAuthorizations.find(ea => ea.id === parseInt(data.exit_authorization_id));
            if (exitAuth) {
                setData(d => ({
                    ...d,
    ...
    const filteredPallets = posPallets.filter(p => {
        ...
        return code.includes(search) || num.includes(search) || labelAr.includes(search) || labelEn.includes(search);
    });
    ...
    const handlePalletKeyDown = (e) => {
        if (showPalletDropdown && filteredPallets.length > 0) {
            if (e.key === "ArrowDown") {
                ...
                if (palletActiveIndex >= 0 && palletActiveIndex < filteredPallets.length) {
                    e.preventDefault();
                    const selected = filteredPallets[palletActiveIndex];
                    setPosPalletId(selected.id.toString());
                    setPalletSearch(`${selected.pallet_code} (${lang === "ar" ? "طبلية" : "Pallet"} ${selected.pallet_number})`);
    ...
    const handleAddPOSRow = () => {
        ...
        if (!posPalletId) {
            setPosRowError(lang === "ar" ? "يرجى اختيار الطبلية." : "Please select pallet.");
        ...
        if (isNaN(qty) || qty <= 0) {
            setPosRowError(lang === "ar" ? "يجب إدخال كمية صحيحة أكبر من الصفر." : "Quantity must be a valid integer greater than zero.");
            qtyInputRef.current?.focus();
            return;
        }

        if (qty > posMaxBalance) {
            setPosRowError(lang === "ar" ? `الكمية المدخلة تجاوزت الرصيد المتاح (${posMaxBalance}).` : `Quantity exceeds available balance (${posMaxBalance}).`);
            qtyInputRef.current?.focus();
            return;
        }
        ...
        if (exists) {
            setPosRowError(lang === "ar" ? "هذا البند مضاف بالفعل في هذا السند." : "This item on this pallet is already added.");
            return;
        }
        ...
        router.put(route("deliveries.update", delivery.id), {
            ...data,
            items: newItems,
            status: "draft",
            redirect_to: "edit"
    ...
    const breadcrumbs = (
        <div className="flex items-center gap-[6px] text-xs text-text-muted">
            ...
            <ChevronRight className={`h-3.5 w-3.5 ${lang === "ar" ? "rotate-180" : ""}`} />
            <Link href={route("deliveries.index")} className="text-primary font-medium hover:underline">
                {lang === "ar" ? "سندات التسليم" : "Deliveries"}
            </Link>
            <ChevronRight className={`h-3.5 w-3.5 ${lang === "ar" ? "rotate-180" : ""}`} />
            <span className="text-primary font-medium">
                {lang === "ar" ? `سند: ${delivery.serial_number}` : `Voucher: ${delivery.serial_number}`}
            </span>
        </div>
    );
    ...
    return (
        <AuthenticatedLayout header={breadcrumbs}>
            <Head title={lang === "ar" ? `سند خروج ${delivery.serial_number}` : `Delivery Voucher ${delivery.serial_number}`} />
            ...
            <div className="max-w-7xl mx-auto pb-8 main-stack-y" dir={lang === "ar" ? "rtl" : "ltr"}>
                ...
                <PageHeader
                    icon={Layers}
                    title={isEdit ? (lang === "ar" ? `تعديل سند خروج: ${delivery.serial_number}` : `Edit Delivery Note: ${delivery.serial_number}`) : (lang === "ar" ? "إنشاء سند خروج جديد" : "Create New Delivery Note")}
                    description={
                    ...
                    actions={
                        <div className="flex items-center gap-1.5">
                            {/* Back to Index link */}
                            <Tooltip text={lang === "ar" ? "العودة للفهرس" : "Back to Index"}>
                                <Link
                                ...
                                >
                                    ...
                                    {showButtonText && <span>{lang === "ar" ? "الفهرس" : "Index"}</span>}
                                </Link>
                            </Tooltip>
                            ...
                            <Tooltip text={lang === "ar" ? "حفظ كمسودة (Ctrl+S)" : "Save Draft (Ctrl+S)"}>
                                <button 
                                    type="button" 
                                    onClick={() => handleFormSubmitWithStatus("draft", "edit")}
                            ...
                            <Tooltip text={lang === "ar" ? "اعتماد وحفظ والعودة للقائمة (Ctrl+Shift+S)" : "Approve, Save & Return to List (Ctrl+Shift+S)"}>
                                <button 
                                    type="button" 
                                    onClick={() => handleFormSubmitWithStatus("approved", "index")}
                                ...
                                >
                                    ...
                                    <span className="hidden sm:inline">{lang === "ar" ? "اعتماد" : "Approve"}</span>
                                </button>
                            </Tooltip>

                            {/* Approve & Print button (Ctrl+E) */}
                            <Tooltip text={lang === "ar" ? "اعتماد وطباعة (Ctrl+E)" : "Approve & Print (Ctrl+E)"}>
                                <button 
                                    type="button" 
                                    onClick={() => handleFormSubmitWithStatus("approved", "print")}
                                    className="rounded-none h-[30px] w-[30px] p-0 flex items-center justify-center bg-surface hover:bg-surface-muted border border-border text-text-muted hover:text-text transition-colors"
                                >
                            ...
                            {isEdit && (
                                <>
                                    ...
                                    <Tooltip text={lang === "ar" ? "حذف السند (Ctrl+D)" : "Delete Voucher (Ctrl+D)"}>
                                        <button 
                                            type="button" 
                ...
                {draftDeliveries.length > 0 && (
                    <div className="bg-amber-500/5 border border-amber-500/25 p-3 rounded-none flex flex-wrap items-center gap-3">
                        <span className="text-xs font-bold text-amber-700 flex items-center gap-1">
                            ...
                            {lang === "ar" ? "المسودات النشطة المعلقة:" : "Active Pending Drafts:"}
                        </span>
                        <div className="flex flex-wrap gap-1.5">
                            {draftDeliveries.map((dr) => {
                                const draftTooltipText = dr.customer
                                    ...
                                    : (lang === "ar" ? "لم يتم تعيين عقد" : "Contract not assigned yet");

                                return (
                                    <Tooltip key={dr.id} text={draftTooltipText}>
                                        <button
                                            type="button"
                                            onClick={() => handleSwitchDraft(dr.id)}
                    ...
                    <div className="lg:col-span-3 space-y-6">
                        ...
                        <div className="bg-surface border border-border shadow-sm rounded-none">
                            <div className="p-4 border-b border-border flex justify-between items-center">
                                <h3 className="font-bold text-xs text-primary uppercase tracking-wider">
                                    {lang === "ar" ? "بيانات المستند والمستلم" : "Voucher & Deliveree Details"}
                                </h3>
                                <button
                                    type="button"
                                ...
                                >
                                    {isGeneralCollapsed ? <ChevronDown className="h-4 w-4" /> : <ChevronUp className="h-4 w-4" />}
                                </button>
                            </div>
                            ...
                            {!isGeneralCollapsed && (
                                <div className="p-5 space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        ...
                                        {/* Exit Authorization */}
                                        <div>
                                            <InputLabel value={lang === "ar" ? "إذن الخروج المرجعي" : "Referential Exit Permit"} />
                                            <select
                                                ...
                                                value={data.exit_authorization_id}
                                                onChange={(e) => setData("exit_authorization_id", e.target.value)}
                                            >
                                                <option value="">{lang === "ar" ? "-- بدون إذن (سحب مباشر) --" : "-- Direct Delivery (No Permit) --"}</option>
                                                {exitAuthorizations.map((ea) => (
                                                    <option key={ea.id} value={ea.id}>
                                                        {ea.serial_number} - {ea.customer?.name}
                                                    </option>
                                                ))}
                                            </select>
                                        </div>
                                        ...
                                        <div className="relative">
                                            <InputLabel value={lang === "ar" ? "العميل المستلم *" : "Customer *"} />
                                            <input
                                                type="text"
                                        ...
                                        {/* Contract */}
                                        <div>
                                            <InputLabel value={lang === "ar" ? "العقد للعميل *" : "Contract *"} />
                                            <select
                                                className="mt-1 block w-full border-border bg-surface text-text text-xs focus:border-primary focus:ring-primary rounded-none h-[38px] px-2.5"
                                                value={data.contract_id}
                                                onChange={(e) => setData("contract_id", e.target.value)}
                                                disabled={!data.customer_id || !!data.exit_authorization_id}
                                                required
                                            >
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        ...
                                        {/* Billing Period */}
                                        <div>
                                            <InputLabel value={lang === "ar" ? "فترة الفوترة النشطة *" : "Active Period *"} />
                                            <select
                                                className="mt-1 block w-full border-border bg-surface text-text text-xs focus:border-primary focus:ring-primary rounded-none h-[38px] px-2.5"
                                            ...
                                            >
                                                ...
                                                {availablePeriods.map((p) => (
                                                    <option key={p.id} value={p.id}>
                                                        {new Date(p.start_date).toLocaleDateString(lang === 'ar' ? 'ar-EG' : 'en-US')} - {new Date(p.end_date).toLocaleDateString(lang === 'ar' ? 'ar-EG' : 'en-US')}
                                                    </option>
                                                ))}
                                            </select>
                                        ...
                                        {/* Written Reference */}
                                        <div>
                                            <InputLabel value={lang === "ar" ? "المرجع الخطي (مطلوب بدون إذن)" : "Written Reference"} />
                                            <TextInput
                                                className="w-full text-xs rounded-none border-border mt-1 h-[38px]"
                                                placeholder={lang === "ar" ? "مثال: ايميل العميل بتاريخ..." : "e.g. Email dated..."}
                                                value={data.written_reference}
                                                onChange={(e) => setData("written_reference", e.target.value)}
                                                required={!data.exit_authorization_id}
                                            />
                                            <InputError message={errors.written_reference} className="mt-1" />
                                        </div>

                                        {/* Delivery Date */}
                                        <div>
                                            <InputLabel value={lang === "ar" ? "تاريخ التسليم والخرج *" : "Delivery Date *"} />
                                            <TextInput
                                                type="date"
                                                className="w-full text-xs rounded-none border-border mt-1 h-[38px]"
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        ...
                                        {/* Representative */}
                                        <div>
                                            <InputLabel value={lang === "ar" ? "مندوب الاستلام المعتمد" : "Authorized Representative"} />
                                            <select
                                                className="mt-1 block w-full border-border bg-surface text-text text-xs focus:border-primary focus:ring-primary rounded-none h-[38px] px-2.5"
                                                value={data.representative_id}
                                                onChange={(e) => setData("representative_id", e.target.value)}
                                            >
                                                <option value="">{lang === "ar" ? "-- مندوب المستلم --" : "-- Customer Rep --"}</option>
                                                {availableRepresentatives.map((rep) => (
                                                    <option key={rep.id} value={rep.id}>
                                                        {rep.name} ({rep.phone_number || "—"})
                                                    </option>
                                                ))}
                                            </select>
                                            <InputError message={errors.representative_id} className="mt-1" />
                                        </div>

                                        {/* Driver */}
                                        <div>
                                            <div className="flex justify-between items-center">
                                                <InputLabel value={lang === "ar" ? "السائق الناقل" : "Carrier Driver"} />
                                                <button
                                                    type="button"
                                                ...
                                                >
                                                    ...
                                                    {lang === "ar" ? "سائق سريع" : "Quick Driver"}
                                                </button>
                                            </div>
                                            <select
                                                className="mt-1 block w-full border-border bg-surface text-text text-xs focus:border-primary focus:ring-primary rounded-none h-[38px] px-2.5"
                                            ...
                                            >
                                                <option value="">{lang === "ar" ? "-- السائق واللوحة --" : "-- Driver & Plate --"}</option>
                                                {driverList.map((d) => (
                                    ...
                                    {/* Notes */}
                                    <div className="pt-2">
                                        <InputLabel value={lang === "ar" ? "ملاحظات وتوجيهات السند" : "Remarks & Directions"} />
                                        <textarea
                                            ...
                                            placeholder={lang === "ar" ? "اكتب تفاصيل إضافية..." : "Write details..."}
                                        />
                        ...
                        <div className="bg-surface border border-primary/20 p-5 shadow-sm rounded-none space-y-4">
                            <h3 className="font-bold text-xs text-primary border-b border-border pb-2 uppercase tracking-wider flex items-center gap-1.5">
                                ...
                                {lang === "ar" ? "شريط سحب البضائع الفوري (POS Bar)" : "Progressive Loading POS Bar"}
                            </h3>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
                                ...
                                <div className="sm:col-span-3 relative">
                                    <InputLabel value={lang === "ar" ? "الطبلية المتاحة (رصيد > 0) *" : "Pallet *"} />
                                    <input
                                        ref={palletSelectRef}
                                        type="text"
                                        ...
                                        placeholder={loadingPallets ? (lang === "ar" ? "جاري التحميل..." : "Loading...") : (lang === "ar" ? "ابحث عن طبلية..." : "Search pallet...")}
                                        value={palletSearch}
                                        onChange={(e) => {
                                            const val = e.target.value;
                                            setPalletSearch(val);
                                        ...
                                        onFocus={() => setShowPalletDropdown(true)}
                                        onBlur={() => setTimeout(() => {
                                            setShowPalletDropdown(false);
                                            setPalletActiveIndex(-1);
                                        }, 200)}
                                    ...
                                    />
                                    {showPalletDropdown && filteredPallets.length > 0 && (
                                        <div className="absolute z-10 w-full bg-surface border border-border shadow-md max-h-48 overflow-y-auto mt-1 divide-y divide-border">
                                            {filteredPallets.map((p, index) => (
                                                <div
                                                    key={p.id}
                                                    className={`p-2 text-xs font-semibold cursor-pointer transition-colors ${
                                                        index === palletActiveIndex 
                                                            ? "bg-primary text-white font-bold" 
                                                            : "text-text hover:bg-slate-100"
                                                    }`}
                                                    onMouseDown={() => {
                                                        setPosPalletId(p.id.toString());
                                                        setPalletSearch(`${p.pallet_code} (${lang === "ar" ? "طبلية" : "Pallet"} ${p.pallet_number})`);
                                                        setShowPalletDropdown(false);
                                                        setPalletActiveIndex(-1);
                                                    }}
                                                >
                                ...
                                <div className="sm:col-span-3">
                                    <InputLabel value={lang === "ar" ? "الصنف على الطبلية *" : "Pallet Item *"} />
                                    <select
                                        ref={itemSelectRef}
                                        ...
                                        onKeyDown={(e) => {
                                            if (e.key === "ArrowRight" || e.key === "ArrowLeft") {
                                                handleKeyNavigation(e, variantSelectRef, palletSelectRef);
                                            }
                                        }}
                                        disabled={loadingItems || !posPalletId}
                                    >
                                        <option value="">{loadingItems ? (lang === "ar" ? "جاري التحميل..." : "Loading...") : (lang === "ar" ? "-- اختر الصنف --" : "-- Select Item --")}</option>
                                        {posItems.map((item) => (
                                ...
                                <div className="sm:col-span-2">
                                    <InputLabel value={lang === "ar" ? "العبوة والوزن *" : "Variant *"} />
                                    <select
                                        ...
                                        onKeyDown={(e) => {
                                            if (e.key === "ArrowRight" || e.key === "ArrowLeft") {
                                                handleKeyNavigation(e, qtyInputRef, itemSelectRef);
                                            }
                                        }}
                                        disabled={loadingVariants || !posItemId}
                                    >
                                        <option value="">{loadingVariants ? (lang === "ar" ? "جاري التحميل..." : "Loading...") : (lang === "ar" ? "-- اختر العبوة --" : "-- Select Variant --")}</option>
                                        {posVariants.map((v) => (
                                            <option key={v.inventory_item_variant_id} value={v.inventory_item_variant_id}>
                                                {displayBilingual(v.variant?.name)} ({v.balance})
                                            </option>
                                        ))}
                                    </select>
                                </div>
                                ...
                                <div className="sm:col-span-2">
                                    <div className="flex justify-between items-center">
                                        <InputLabel value={lang === "ar" ? "الكمية *" : "Qty *"} />
                                        {posMaxBalance > 0 && (
                                    ...
                                    <input
                                        ...
                                        onKeyDown={(e) => {
                                            if (e.key === "." || e.key === "," || e.key === "e" || e.key === "-" || e.key === "+") {
                                                ...
                                                handleKeyNavigation(e, insertBtnRef, variantSelectRef);
                                            }
                                        }}
                                ...
                                <div className="sm:col-span-2">
                                    <button
                                        ...
                                        onKeyDown={(e) => {
                                            if (e.key === "ArrowRight" || e.key === "ArrowLeft") {
                                                handleKeyNavigation(e, null, qtyInputRef);
                                            }
                                        }}
                                    >
                        ...
                        <div className="bg-surface border border-border p-5 shadow-sm rounded-none space-y-4">
                            <h3 className="font-bold text-xs text-primary border-b border-border pb-2 uppercase tracking-wider">
                                {lang === "ar" ? "البنود المنصرفة بالسند" : "Pallets Deliveries List"}
                            </h3>
                                ...
                                <table className="w-full text-xs text-start border border-border">
                                    <thead className="bg-surface-muted text-text-muted font-bold border-b border-border">
                                        <tr>
                                            ...
                                            <th className="px-3 py-2 text-start">{lang === "ar" ? "رقم الطبلية" : "Pallet No."}</th>
                                            <th className="px-3 py-2 text-start">{lang === "ar" ? "نوع الطبلية" : "Pallet Size"}</th>
                                            ...
                                            <th className="px-3 py-2 text-start">{lang === "ar" ? "الدرجة" : "Quality"}</th>
                                            <th className="px-3 py-2 text-end text-danger">{lang === "ar" ? "الكمية المنصرفة" : "Qty Out"}</th>
                                    ...
                                    <tbody className="divide-y divide-border">
                                        {data.items.length === 0 ? (
                                            <tr>
                                                <td colSpan="8" className="px-3 py-6 text-center text-text-muted font-bold">
                                                    {lang === "ar" ? "لا توجد طبالي منصرفة في السند بعد. استخدم الشريط السريع بالأعلى لإدراج الطبالي." : "No delivery items added yet. Use the POS bar to load pallets."}
                                                </td>
                                            </tr>
                                        ) : (
                                            data.items.map((item, idx) => {
                                                ...
                                                return (
                                                    <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                                        ...
                                                        <td className="px-3 py-2.5 text-text-muted font-semibold">{item.pallet_size}</td>
                                                        <td className="px-3 py-2.5 font-bold text-text">{displayBilingual(inventoryItem?.name)}</td>
                                                        <td className="px-3 py-2.5 text-text-muted font-semibold">{displayBilingual(variant?.name)}</td>
                                                        <td className="px-3 py-2.5 text-text-muted">{displayBilingual(variant?.quality) || "—"}</td>
                                                        <td className="px-3 py-2.5 font-mono font-extrabold text-end text-danger-600">
                                                            {parseInt(item.quantity_out, 10)}
                                                        </td>
                                    ...
                                    {loadingStats ? (
                                        ...
                                        <>
                                            ...
                                            <div className="flex justify-between border-b border-border/40 pb-1.5">
                                                ...
                                                <span className="font-bold font-mono text-emerald-600">{contractStats.available_pallets || "0"} {lang === "ar" ? "طبلية" : "pallets"}</span>
                                            </div>
                                            <div className="flex justify-between border-b border-border/40 pb-1.5">
                                                <span className="text-text-muted">{lang === "ar" ? "تاريخ انتهاء الصلاحية:" : "End Date:"}</span>
                                        ...
                                        <p className="text-text-muted italic">{lang === "ar" ? "يرجى تحديد عقد لعرض الإحصائيات." : "Please select contract to load details."}</p>
                                    )}
                                </div>
                            )}
                        </div>
                ...
                <div className="p-6 font-main" dir={lang === "ar" ? "rtl" : "ltr"}>
                    ...
                    <form onSubmit={handleCreateDriver} className="space-y-3.5">
                        ...
                        <div className="flex justify-end gap-2 pt-2">
                            ...
                            <Tooltip text={lang === "ar" ? "إدراج السائق" : "Save Driver"}>
                                <button
                                    type="submit"
                                    ...
                                    {showButtonText && (
                                        <span>
                                            {addingDriver
                                                ...
                                                : (lang === "ar" ? "إدراج السائق" : "Save Driver")}
                                        </span>
                                    )}
                                </button>
                            </Tooltip>
            ...
            <Modal show={isDeleteModalOpen} onClose={() => { setIsDeleteModalOpen(false); setDeletePassword(""); setDeleteError(""); }}>
                <div className="p-6 font-main" dir={lang === "ar" ? "rtl" : "ltr"}>
                    <div className="flex items-center gap-3 text-danger mb-4">
                        <ShieldAlert className="h-6 w-6" />
                        <h3 className="text-sm font-bold">
                            {lang === "ar" ? "تأكيد حذف مسودة سند التسليم" : "Confirm Delete Draft Note"}
                        </h3>
                    </div>
                    ...
                    <form onSubmit={confirmDelete} className="space-y-4">
                        <div>
                            <InputLabel value={lang === "ar" ? "رمز التأكيد الأمني (كلمة المرور)" : "Security Password"} />
                            <TextInput
                                type="password"
                                className="w-full text-xs mt-1"
                                placeholder="••••••••"
