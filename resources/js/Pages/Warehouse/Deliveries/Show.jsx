...
export default function Show({ delivery }) {
            ...
            },
            {
                ...
                onError: (errs) => {
                    ...
                    if (errs.error) {
                        ...
                        setErrorMsg(lang === "ar" ? "تعذر حذف السند." : "Failed to delete.");
                    }
                }
            }
        );
    ...
    const breadcrumbs = (
        <div className="flex items-center gap-[6px] text-xs text-text-muted">
            ...
            <Link href={route("deliveries.index")} className="hover:text-primary transition-colors">
                {lang === "ar" ? "سندات التسليم" : "Goods Delivery Notes"}
            </Link>
            <ChevronRight className={`h-3.5 w-3.5 ${lang === "ar" ? "rotate-180" : ""}`} />
            <span className="text-primary font-medium">{delivery.serial_number}</span>
        </div>
    );

    return (
        <AuthenticatedLayout header={breadcrumbs}>
            <Head title={lang === "ar" ? `تفاصيل سند الخروج: ${delivery.serial_number}` : `Delivery Voucher Details: ${delivery.serial_number}`} />
                ...
                <PageHeader
                    ...
                    title={
                        <div className="flex items-center gap-3">
                            <span className="font-extrabold text-lg text-text">{lang === "ar" ? `سند تسليم خروج بضاعة: ${delivery.serial_number}` : `Delivery Voucher: ${delivery.serial_number}`}</span>
                            <span
                            ...
                            >
                                {delivery.status === "approved" ? (
                                    <span className="flex items-center gap-0.5">
                                        ...
                                        {lang === "ar" ? "معتمد ومغلق" : "Approved & Locked"}
                                    </span>
                                ) : (
                                    <span className="flex items-center gap-0.5">
                    ...
                    actions={
                        <div className="flex flex-wrap items-center gap-1.5">
                            {delivery.status === "draft" && (
                                <>
                                    <Tooltip text={lang === "ar" ? "تعديل السند" : "Edit Note"}>
                                        <Link
                                    ...
                                    <Tooltip text={lang === "ar" ? "اعتماد السند" : "Approve Note"}>
                                        <button
                                            onClick={() => {
                                                setErrorMsg("");
                                                setSecurePassword("");
                            ...
                            {delivery.status === "approved" && (
                                <Tooltip text={lang === "ar" ? "إعادة فتح السند" : "Reopen Note"}>
                                    <button
                                        onClick={() => {
                                            setErrorMsg("");
                                            setSecurePassword("");
                                    ...
                                    >
                                        ...
                                        {showButtonText && <span>{lang === "ar" ? "إعادة فتح" : "Reopen"}</span>}
                                    </button>
                                </Tooltip>
                            )}
                            ...
                            <Tooltip text={lang === "ar" ? "طباعة" : "Print"}>
                                <a
                                    href={route("deliveries.print", delivery.id)}
                                    target="_blank"
                            ...
                            <Tooltip text={lang === "ar" ? "رجوع للقائمة" : "Back to List"}>
                                <Link
                                    href={route("deliveries.index")}
                                    className={`border border-border bg-surface text-text hover:bg-surface-muted rounded-none flex items-center justify-center h-[30px] transition-all gap-1.5 ${showButtonText ? 'px-3' : 'w-[30px] p-0'}`}
                                >
                                    <ArrowRight className={`h-4 w-4 ${lang === "ar" ? "" : "rotate-180"}`} />
                                    {showButtonText && <span>{lang === "ar" ? "العودة" : "Back"}</span>}
                                </Link>
                            </Tooltip>
                        </div>
                    }
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-surface border border-border p-5 shadow-sm rounded-none">
                            <div className="flex items-center gap-2 border-b border-border pb-3 mb-4">
                                ...
                                <h3 className="font-bold text-xs text-primary uppercase tracking-wider">
                                    {lang === "ar" ? "بيانات السند والعميل" : "Voucher & Customer Information"}
                                </h3>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 text-xs">
                                <div>
                                    <span className="text-text-muted block font-medium mb-0.5">{lang === "ar" ? "العميل المستلم:" : "Customer:"}</span>
                                    <span className="text-text font-bold text-sm">{delivery.customer?.name}</span>
                                </div>

                                <div>
                                    <span className="text-text-muted block font-medium mb-0.5">{lang === "ar" ? "العقد المرتبط:" : "Linked Contract:"}</span>
                                    {delivery.contract ? (
                                        <Link 
                                            href={route("contracts.show", delivery.contract_id)}
                                ...
                                <div>
                                    <span className="text-text-muted block font-medium mb-0.5">{lang === "ar" ? "فترة الفوترة النشطة:" : "Billing Period:"}</span>
                                    <span className="text-text font-semibold">
                                        {lang === "ar" ? "الفترة" : "Period"} {delivery.period?.period_number}{" "}
                                        <span className="text-text-muted font-mono font-normal">
                                            ({delivery.period?.start_date} {lang === "ar" ? "إلى" : "to"} {delivery.period?.end_date})
                                        </span>
                                    </span>
                                </div>

                                <div>
                                    <span className="text-text-muted block font-medium mb-0.5">{lang === "ar" ? "تاريخ التسليم الفعلي:" : "Delivery Date:"}</span>
                                ...
                                <div>
                                    <span className="text-text-muted block font-medium mb-0.5">{lang === "ar" ? "المرجع / إذن الخروج:" : "Exit Permit / Reference:"}</span>
                                    {delivery.exit_authorization ? (
                                        <span className="text-text font-bold text-primary flex items-center gap-1">
                                            <FileCheck className="h-4 w-4" />
                                            {delivery.exit_authorization.serial_number}
                                        </span>
                                    ) : (
                                        <span className="text-text font-semibold">{delivery.written_reference || "—"}</span>
                                    )}
                                </div>

                                <div>
                                    <span className="text-text-muted block font-medium mb-0.5">{lang === "ar" ? "مندوب الاستلام:" : "Representative:"}</span>
                                    {delivery.representative ? (
                                        <span className="text-text font-semibold">
                                            {delivery.representative.name}{" "}
                                            <span className="text-text-muted font-mono font-normal">
                                                ({delivery.representative.phone_number})
                                            </span>
                                        </span>
                                    ) : (
                                        <span className="text-text-muted font-semibold">{lang === "ar" ? "لا يوجد مندوب مسجل" : "None"}</span>
                                    )}
                                </div>

                                <div className="md:col-span-2">
                                    <span className="text-text-muted block font-medium mb-0.5">{lang === "ar" ? "ملاحظات إضافية:" : "Notes / Remarks:"}</span>
                                    <p className="text-text bg-background border border-border p-3 font-semibold text-xs leading-relaxed min-h-[45px]">
                                        {delivery.notes || (lang === "ar" ? "لا توجد ملاحظات." : "No notes recorded.")}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-surface border border-border p-5 shadow-sm rounded-none">
                            <div className="flex items-center gap-2 border-b border-border pb-3 mb-4">
                                ...
                                <h3 className="font-bold text-xs text-primary uppercase tracking-wider">
                                    {lang === "ar" ? "البضائع والكميات المنصرفة" : "Delivered Items & Pallets List"}
                                </h3>
                            </div>
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-xs text-start">
                                    <thead className="bg-surface-muted/50 text-text-muted font-bold border-b border-border">
                                        <tr>
                                            <th className="px-3 py-2 text-start">{lang === "ar" ? "رقم البند" : "Row #"}</th>
                                            <th className="px-3 py-2 text-start">{lang === "ar" ? "الصنف المخزني" : "Inventory Item"}</th>
                                            <th className="px-3 py-2 text-start">{lang === "ar" ? "الشكل/البديل" : "Variant"}</th>
                                            <th className="px-3 py-2 text-start">{lang === "ar" ? "رقم الطبلية" : "Pallet Number"}</th>
                                            <th className="px-3 py-2 text-end text-danger">{lang === "ar" ? "الكمية المنصرفة" : "Qty Delivered"}</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-border">
                                        {delivery.inventory_entries?.map((entry, index) => (
                                            <tr key={entry.id} className="hover:bg-surface-muted/10">
                                                <td className="px-3 py-2.5 font-mono text-text-muted">{index + 1}</td>
                                                <td className="px-3 py-2.5 font-bold text-text">{displayBilingual(entry.inventory_item?.name)}</td>
                                                <td className="px-3 py-2.5 text-text-muted">
                                                    {displayBilingual(entry.variant?.name)}
                                                    {entry.variant?.quality ? ` (${displayBilingual(entry.variant.quality)})` : ""}
                                                </td>
                                                ...
                                                <td className="px-3 py-2.5 font-mono font-extrabold text-end text-danger-600">
                                                    {Math.round(parseFloat(entry.quantity_out)).toLocaleString()}
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        ...
                        <div className="bg-surface border border-border p-5 shadow-sm rounded-none">
                            <div className="flex items-center gap-2 border-b border-border pb-3 mb-4">
                                ...
                                <h3 className="font-bold text-xs text-primary uppercase tracking-wider">
                                    {lang === "ar" ? "بيانات السائق الناقل" : "Carrier Driver Info"}
                                </h3>
                            </div>
                            ...
                            {delivery.driver ? (
                                <div className="space-y-3 text-xs">
                                    <div>
                                        <span className="text-text-muted block font-medium">{lang === "ar" ? "اسم السائق:" : "Driver Name:"}</span>
                                        <span className="text-text font-bold">{delivery.driver.name}</span>
                                    </div>
                                    <div>
                                        <span className="text-text-muted block font-medium">{lang === "ar" ? "رقم الجوال:" : "Phone Number:"}</span>
                                        <span className="text-text font-mono font-bold">{delivery.driver.phone_number || "—"}</span>
                                    </div>
                                    <div>
                                        <span className="text-text-muted block font-medium">{lang === "ar" ? "رقم الهوية / الإقامة:" : "ID / Iqama No.:"}</span>
                                        <span className="text-text font-mono font-bold">{delivery.driver.id_number || "—"}</span>
                                    </div>
                                    <div>
                                        <span className="text-text-muted block font-medium">{lang === "ar" ? "رقم اللوحة:" : "Plate Number:"}</span>
                                        <span className="text-text font-bold font-mono">{delivery.driver.vehicle_plate || "—"}</span>
                                    </div>
                                    <div>
                                        <span className="text-text-muted block font-medium">{lang === "ar" ? "نوع السيارة:" : "Vehicle Type:"}</span>
                                        <span className="text-text font-semibold">{delivery.driver.vehicle_type || "—"}</span>
                                    </div>
                                </div>
                            ) : (
                                <div className="py-4 text-center text-xs text-text-muted">
                                    {lang === "ar" ? "لم يتم تحديد بيانات سائق." : "No carrier driver assigned."}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="bg-surface border border-border p-5 shadow-sm rounded-none">
                            <div className="flex items-center gap-2 border-b border-border pb-3 mb-4">
                                ...
                                <h3 className="font-bold text-xs text-primary uppercase tracking-wider">
                                    {lang === "ar" ? "سجل التعديلات والعمليات" : "Modification Log"}
                                </h3>
                            </div>
                            ...
                            {delivery.history && delivery.history.length > 0 ? (
                                    ...
                                    {delivery.history.map((log, idx) => (
                                        <div key={idx} className="relative">
                                            ...
                                            <div className="font-mono text-[10px] text-text-muted">
                                                {new Date(log.date).toLocaleString(lang === "ar" ? "ar-EG" : "en-US")}
                                            </div>
                                            <div className="font-bold text-text mt-0.5">{log.user}</div>
                                ...
                                <div className="py-4 text-center text-xs text-text-muted">
                                    {lang === "ar" ? "لا توجد تعديلات سابقة مسجلة." : "No previous adjustments recorded."}
                                </div>
                            )}
                        </div>
            ...
            <Modal show={isDeleteModalOpen} onClose={() => setDeleteModalOpen(false)} maxWidth="md">
                <form onSubmit={handleDelete} className="p-6 space-y-4 text-start" dir={lang === "ar" ? "rtl" : "ltr"}>
                    <div className="flex items-center gap-2 border-b border-border pb-3">
                        <ShieldAlert className="h-6 w-6 text-danger animate-bounce" />
                        <h3 className="font-bold text-lg text-text">
                            {lang === "ar" ? "تأكيد حذف سند التسليم" : "Confirm Voucher Deletion"}
                        </h3>
                    </div>
                    ...
                    <div className="bg-surface-muted/50 p-3 border border-border text-xs font-mono rounded-none">
                        <div>
                            <span className="font-bold text-text-muted">{lang === "ar" ? "رقم السند: " : "Voucher Serial: "}</span>
                            <span className="text-text font-bold">{delivery.serial_number}</span>
                        </div>
                        <div className="mt-1">
                            <span className="font-bold text-text-muted">{lang === "ar" ? "العميل: " : "Customer: "}</span>
                            <span className="text-text font-bold">{delivery.customer?.name}</span>
                        </div>
                    </div>
                ...
                <form onSubmit={handleReopen} className="p-6 space-y-4 text-start" dir={lang === "ar" ? "rtl" : "ltr"}>
                    ...
                    <div>
                        ...
                        <TextInput
                            ...
                            placeholder={lang === "ar" ? "مثال: تعديل خطأ في كمية الصرف..." : "e.g., correcting delivery quantity..."}
                            required
                        />
                    </div>
            ...
            <Modal show={isApproveModalOpen} onClose={() => setApproveModalOpen(false)} maxWidth="md">
                <div className="p-6 space-y-4 text-start" dir={lang === "ar" ? "rtl" : "ltr"}>
                    <div className="flex items-center gap-2 border-b border-border pb-3">
                        ...
                        <h3 className="font-bold text-lg text-text">
                            {lang === "ar" ? "اعتماد وإغلاق سند التسليم" : "Approve & Lock Voucher"}
                        </h3>
                    </div>
                    ...
                    <div className="flex items-center justify-end gap-2 border-t border-border pt-4">
                        ...
                        {/* Approve → Print */}
                        <Tooltip text={lang === "ar" ? "اعتماد وطباعة" : "Approve & Print"}>
                            <button
                                type="button"
                                disabled={processingAction}
                            ...
                            >
                                ...
                                {showButtonText && (
                                    <span>
                                        {processingAction
                                            ? "..."
                                            : (lang === "ar" ? "اعتماد وطباعة" : "Approve & Print")}
                                    </span>
                                )}
                            </button>
                        </Tooltip>
