...
export default function Print({ rearrangement, companySettings = {} }) {
    ...
    const getPalletFormatted = (item) => {
        const palletNum = item.pallet?.pallet_number || item.pallet_number || "—";
    ...
    for (let p = 0; p < totalPagesCount; p++) {
        const pageItems = itemsList.slice(p * ITEMS_PER_PAGE, (p + 1) * ITEMS_PER_PAGE);
        pageChunks.push({
            pageIndex: p + 1,
                        ...
                        <div className="space-y-3 print:space-y-2">
                            ...
                            <div className="flex justify-between items-start border-b border-gray-400 pb-2">
                                ...
                                <div className="text-end space-y-0.5">
                                    <h1 className="text-sm font-extrabold uppercase tracking-wide text-black">
                                        {lang === "ar" ? "سند ترتيب ونقل طبالي" : "Pallet Rearrangement Note"}
                                    </h1>
                            ...
                            <div className="grid grid-cols-2 gap-x-8 gap-y-1.5 text-[11px] border-b border-gray-300 pb-2">
                                <div className="space-y-1">
                                    ...
                                    <div className="flex gap-2">
                                        <span className="font-bold text-gray-600 w-24 shrink-0">{lang === "ar" ? "سبب الترتيب:" : "Reason:"}</span>
                                        <span className="font-medium text-gray-900">{rearrangement.reason || "—"}</span>
                                    </div>
                                </div>

                                <div className="space-y-1">
                                    <div className="flex gap-2">
                                        <span className="font-bold text-gray-600 w-24 shrink-0">{lang === "ar" ? "المشرف / المسؤول:" : "Officer:"}</span>
