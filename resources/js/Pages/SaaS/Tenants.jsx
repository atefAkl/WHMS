...
export default function Tenants({
    ...
    const [activeTab, setActiveTab] = useState(initialTab || "tenants"); // 'tenants' or 'requests'
    ...
    const handleTabChange = (tabName) => {
        ...
        let url = route("saas.tenants.index");
        if (tabName === "requests") {
            url = route("saas.tenants.requests");
        }
    ...
    const breadcrumbs = (
        <div className="flex items-center gap-2 text-[12px] text-text-muted">
            ...
            <span className="text-primary font-medium">
                {lang === "ar" ? "إدارة الـ SaaS" : "SaaS Management"}
            </span>
        </div>
    );
    ...
    return (
        <AuthenticatedLayout header={breadcrumbs}>
            <Head
                title={
                    lang === "ar"
                        ? "إدارة المستأجرين والطلبات"
                        : "Tenants & Requests Management"
                }
            />
                    ...
                    <PageHeader
                        ...
                        description={
                            <p className="text-xs text-text-muted mt-0.5">
                                {lang === "ar"
                                    ? "إدارة الاشتراكات السحابية ومراجعة طلبات الانضمام الجديدة."
                                    : "Manage cloud subscriptions and review new joining requests."}
                            </p>
                        }
                        actions={
                            <div className="flex bg-surface-muted p-0.5 rounded-lg border border-border">
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all flex items-center gap-1.5 ${activeTab === "tenants" ? "bg-white shadow-sm text-primary" : "text-text-muted hover:text-text"}`}
                                >
                                ...
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all flex items-center gap-1.5 ${activeTab === "requests" ? "bg-white shadow-sm text-primary" : "text-text-muted hover:text-text"}`}
                                >
                                    ...
                                    {lang === "ar"
                                        ...
                                        : "Pending Requests"}
                                    {requests.length > 0 && (
                                        <span className="bg-amber-500 text-white px-1.5 py-0.5 rounded-full text-[10px] animate-pulse">
                                            {requests.length}
                                        </span>
                                    )}
                                </button>
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                                {[
                                    ...
                                    {
                                        label: "النطاقات الفرعية",
                                        value: kpis.active_subdomains,
                                        icon: Globe,
                                        color: "text-cyan-500",
                                        bg: "bg-cyan-500/10",
                                    },
                                    {
                                        label: "الإيرادات السحابية",
                                    ...
                                    <div
                                        key={i}
                                        className="rounded-xl border border-border bg-surface p-3.5 shadow-sm flex items-center gap-4 transition-all hover:shadow-md"
                                    >
                                        <div
                                        ...
                                        <div>
                                            ...
                                            <p className="text-2xl font-black text-text leading-tight">
                                                {kpi.value}
                                            </p>
                                        </div>
                                    </div>
                            ...
                            <div className="rounded-xl border border-border bg-surface shadow-sm overflow-hidden">
                                <div className="p-4 border-b border-border flex flex-col sm:flex-row gap-4 justify-between items-center bg-surface-muted/20">
                                    <div className="relative w-full sm:w-80">
                                        <Search
                                            className={`absolute top-1/2 -translate-y-1/2 h-4 w-4 text-text-muted ${lang === "ar" ? "right-3" : "left-3"}`}
                                        />
                                        <input
                                            type="text"
                                            placeholder={
                                                lang === "ar"
                                                    ? "بحث باسم المنشأة أو النطاق..."
                                                    : "Search..."
                                            }
                                            value={searchQuery}
                                            onChange={(e) =>
                                        ...
                                        <tbody className="divide-y divide-border">
                                            {filteredTenants.map((tenant) => (
                                                ...
                                                >
                                                    ...
                                                    <td className="py-4 px-6">
                                                        <span
                                                            className={`px-2.5 py-1 rounded-full text-[10px] font-bold flex items-center gap-1.5 w-fit ${tenant.status === "نشط" ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}
                                                        >
                                                            <span
                                                                className={`h-1.5 w-1.5 rounded-full ${tenant.status === "نشط" ? "bg-emerald-500 animate-pulse" : "bg-rose-500"}`}
                                                            ></span>
                                                            {tenant.status}
                                                        </span>
                                                    </td>
                            ...
                            {requests.length === 0 ? (
                                <div className="bg-white border border-border rounded-xl p-12 text-center space-y-4">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-text-muted">
                                            ستظهر هنا طلبات تسجيل المنشآت
                                            الجديدة بمجرد إرسالها من صفحة
                                            الهبوط.
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="grid gap-4">
                                    {requests.map((request) => (
                                        <div
                                            key={request.id}
                                            className="bg-white border border-border rounded-xl p-4 shadow-sm hover:shadow-md transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 border-r-4 border-r-amber-500"
                                        >
                                            ...
                                            <div className="flex items-center gap-2 shrink-0">
                                                ...
                                                <PrimaryButton
                                                    ...
                                                    className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 rounded-lg text-xs font-bold shadow-md shadow-indigo-100"
                                                    disabled={processing}
                                                >
                            ...
                            <div className="space-y-4">
                                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
                                    <div className="flex items-center justify-between text-sm">
                                        ...
                                        <span className="font-mono font-black text-primary select-all">
                                ...
                                <div className="bg-amber-50 border border-amber-200 p-3 rounded-xl space-y-2">
                                    ...
                                    <p className="text-[11px] text-amber-700 leading-relaxed">
                                        لتتمكن من فتح هذا النطاق على جهازك، يجب
                                        إضافة السطر التالي لملف{" "}
                                    ...
                                    <div
                                        className="bg-slate-900 text-emerald-400 p-3 rounded-xl font-mono text-xs select-all text-left"
                                        dir="ltr"
                                    >
