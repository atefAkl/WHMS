...
import { Layers, Plus, Edit, Trash2, Home, ChevronRight, FolderTree } from 'lucide-react';
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
                        <div>
                            ...
                            <p className="text-sm text-text-muted mt-1">
                                {lang === 'ar' ? 'إدارة الهيكل التنظيمي لتصنيفات العملاء' : 'Manage customer categories hierarchy'}
                            </p>
                        </div>
                    ...
                    <div className="rounded-xl border border-border bg-surface shadow-sm overflow-hidden flex flex-col">
                        <div className="border-b border-border p-4 bg-surface-muted/30 flex items-center justify-between">
                            <h3 className="font-semibold text-text flex items-center gap-2">
                                ...
                                {lang === 'ar' ? 'المخطط العام للتصنيفات' : 'Categories Hierarchy'}
                            </h3>
                            <span className="text-sm text-text-muted font-medium">{categories.total} {lang === 'ar' ? 'تصنيف' : 'Categories'}</span>
                        </div>
                                    ...
                                    <table className="min-w-full divide-y divide-border">
                                        <thead className="bg-surface-muted/50">
                                            <tr>
                                                ...
                                                <th scope="col" className="px-6 py-3 text-start text-xs font-semibold uppercase tracking-wider text-text-muted">{t.columns.parent}</th>
                                        ...
                                        <tbody className="divide-y divide-border bg-surface">
                                            {categories.data.map((category) => (
                                                <tr key={category.id} className="transition-colors hover:bg-surface-muted/50">
                                                    <td className="whitespace-nowrap px-6 py-4">
                                                        <div className="flex items-center gap-3">
                                                            <div className={category.parent_id ? 'h-2 w-2 rounded-full bg-emerald-500' : 'h-2 w-2 rounded-full bg-primary'} />
                                                            <div>
                                                                <div className="text-sm font-bold text-text">{lang === 'ar' ? category.name_ar : category.name_en}</div>
                                                                <div className="text-xs text-text-muted">{lang === 'ar' ? category.name_en : category.name_ar}</div>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="whitespace-nowrap px-6 py-4 text-sm text-text font-medium">
                                                        {category.parent ? (
                                                            <span className="inline-flex items-center rounded-md bg-surface-muted px-2 py-1 text-xs font-medium text-text-muted ring-1 ring-inset ring-border">
                                                                {lang === 'ar' ? category.parent.name_ar : category.parent.name_en}
                                                            </span>
                                                        ) : (
                                                    ...
                                                    <td className="whitespace-nowrap px-6 py-4 text-end text-sm font-medium">
                                                        <div className="flex items-center justify-end gap-2">
                                                            <Tooltip text={lang === 'ar' ? 'تعديل' : 'Edit'}>
                                                                <button onClick={() => openEditModal(category)} className="p-1.5 rounded-md text-text-muted hover:text-primary hover:bg-primary/10 transition-colors">
                                                                    <Edit className="h-4 w-4" />
                                                                </button>
                                                            </Tooltip>
                                                            <Tooltip text={lang === 'ar' ? 'حذف' : 'Delete'}>
                                                                <button onClick={() => openDeleteModal(category)} className="p-1.5 rounded-md text-text-muted hover:text-danger hover:bg-danger/10 transition-colors">
                        ...
                        <Pagination links={categories.links} total={categories.total} from={categories.from} to={categories.to} />
                    </div>
                </div>
            </div>
            ...
            <Modal show={isFormModalOpen} onClose={closeModals} maxWidth="md">
                <form onSubmit={submitForm} className="p-6">
                    <h2 className="text-lg font-bold text-text mb-6">
                        {categoryToEdit ? (lang === 'ar' ? 'تعديل التصنيف' : 'Edit Category') : t.add}
                    </h2>
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            <InputLabel htmlFor="parent_id" value={lang === 'ar' ? 'التصنيف الرئيسي (اختياري)' : 'Parent Category (Optional)'} />
                            <select 
                                id="parent_id"
                            ...
                            >
                                <option value="">{lang === 'ar' ? '-- بدون تصنيف رئيسي --' : '-- No Parent --'}</option>
            ...
            <Modal show={isDeleteModalOpen} onClose={closeModals} maxWidth="sm">
                <div className="p-6 text-center">
                    ...
                    <div className="flex justify-center gap-3">
                        <SecondaryButton type="button" onClick={closeModals}>{lang === 'ar' ? 'إلغاء' : 'Cancel'}</SecondaryButton>
                        <DangerButton onClick={deleteCategory} disabled={processing}>{lang === 'ar' ? 'حذف' : 'Delete'}</DangerButton>
                    </div>
                </div>
            </Modal>
        </AuthenticatedLayout>
