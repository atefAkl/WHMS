...
export default function StorageItems({ items }) {
    ...
    return (
        <AuthenticatedLayout header={breadcrumbs}>
                                        ...
                                        <tr key={item.id} className="hover:bg-surface-muted/30 transition-colors">
                                            ...
                                            <td className="px-4 py-3 text-end space-x-1 rtl:space-x-reverse">
                                                ...
                                                <Tooltip text={lang === 'ar' ? 'حذف' : 'Delete'}>
                                                    <button onClick={() => requestDelete(route('settings.storage-items.destroy', item.id), item)} className={`p-1.5 rounded-md text-text-muted hover:text-danger hover:bg-danger/10 transition-colors ${item.id <= 2 ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={item.id <= 2}>
            ...
            <ConfirmationModal
                show={!!itemToDelete}
                title={lang === "ar" ? "حذف وحدة التخزين" : "Delete Storage Item"}
                message={
